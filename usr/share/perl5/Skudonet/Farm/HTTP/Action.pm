	...
	foreach my $farm_filename ( @farm_configfiles )
	{
		if ( -e "$farm_filename" )
		{
			...
			if ( $farm_filename eq "$configdir\/$farm_name\_proxy.cfg" )
			{
				...
				#\tErr414 "/usr/local/skudonet/config/BasekitHTTP_Err414.html"
				#\tErr500 "/usr/local/skudonet/config/BasekitHTTP_Err500.html"
				#\tErr501 "/usr/local/skudonet/config/BasekitHTTP_Err501.html"
				#\tErr503 "/usr/local/skudonet/config/BasekitHTTP_Err503.html"
				#\t#Service "BasekitHTTP"
				#NfMarks (for each backend)
				grep { s/^(\s*Name\s+"?)$farm_name/$1$new_farm_name/ } @configfile;
				grep {
					s/\tErrWAF "\/usr\/local\/skudonet\/config\/${farm_name}_ErrWAF.html"/\tErrWAF "\/usr\/local\/skudonet\/config\/${new_farm_name}_ErrWAF.html"/
				} @configfile;
				grep {
					s/\tErr414 "\/usr\/local\/skudonet\/config\/${farm_name}_Err414.html"/\tErr414 "\/usr\/local\/skudonet\/config\/${new_farm_name}_Err414.html"/
				} @configfile;
				grep {
					s/\tErr500 "\/usr\/local\/skudonet\/config\/${farm_name}_Err500.html"/\tErr500 "\/usr\/local\/skudonet\/config\/${new_farm_name}_Err500.html"/
				} @configfile;
				grep {
					s/\tErr501 "\/usr\/local\/skudonet\/config\/${farm_name}_Err501.html"/\tErr501 "\/usr\/local\/skudonet\/config\/${new_farm_name}_Err501.html"/
				} @configfile;
				grep {
					s/\tErr503 "\/usr\/local\/skudonet\/config\/${farm_name}_Err503.html"/\tErr503 "\/usr\/local\/skudonet\/config\/${new_farm_name}_Err503.html"/
				} @configfile;
