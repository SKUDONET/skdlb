...
sub apply_backup
{
	...
	if ( $b_version ne $sys_version )
	{
		...
		{
			my $msg =
			  "The backup version ($b_version) is different to the Skudonet version ($sys_version). The parameter 'force' must be used to force the backup applying.";
			&httpErrorResponse( code => 400, desc => $desc, msg => $msg );
		}
		else
		{
			&zenlog(
				"Applying The backup version ($b_version) is different to the Skudonet version ($sys_version)."
			);
		}
	}
