...
function getVersion() {
	...
	if [ -z ${version} ]; then
		if [ -f $bash_aliases_file ]; then
			...
			# alias
			version=`skudonet-tag_master-5.12`
		else
...
getVersion
pkgname_prefix="skudonet_${version}_${arch}"
...
globalconftpl='usr/local/skudonet/share/global.conf.template'
...
if [[ $devel == "false" ]]; then
	...
	# Don't include API 3
	find -L usr/local/skudonet/bin \
			usr/share/perl5/Skudonet \
			usr/local/skudonet/www/zapi/v3.1 \
			usr/local/skudonet/www/zapi/v4.0 \
			-type f \
